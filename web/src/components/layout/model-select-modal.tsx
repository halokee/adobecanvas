...
export function ModelSelectModal({ open, channel, selectedNames, onConfirm, onClose }: { open: boolean; channel: ModelChannel | null; selectedNames: string[]; onConfirm: (names: string[]) => void; onClose: () => void }) {
    ...
    const fetchAdobeModels = async () => {
        setAdobeLoading(true);
        try {
            const res = await fetch("/api/models");
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            const data = (await res.json()) as BackendModels;
            const names: string[] = [];
            for (const m of data.images) names.push(m.id);
            for (const m of data.videos) names.push(m.id);
            ...
            setSelected((prev) => {
                ...
                unique.forEach((name) => next.add(name));
                return next;
            });
            setActiveTab("new");
            message.success(t("config.modelSelect.fetched", { count: unique.length }));
        } catch (error) {
            message.error(error instanceof Error ? error.message : t("config.modelSelect.fetchFailed"));
        } finally {
    ...
    return (
        ...
        >
            <div className="flex flex-wrap items-center gap-3">
                ...
                <Button icon={<Layers className="size-4" />} loading={adobeLoading} onClick={() => void fetchAdobeModels()}>
                    {t("config.modelSelect.fetchAdobe")}
                </Button>
            </div>
